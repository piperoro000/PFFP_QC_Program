...
def pass_report_csv(file_path , Qcutoff, file_naming):
    ...
    """
    #  Result Dictionary
    result = {"Read Number": [], "Read ID": [], "Barcode": [], "QC": []}
    
    # Filtering Reads and Extracting IDs
    good_id = filter_goodread(file_path, Qcutoff)  # {nums_rec: Q_score}
    reads_id = extract_fastq_ids(file_path)  # [(nums_id, rec_ids)]

    # Loop over reads to append to result dictionary
...
def write_fastq(file_path, file_naming, Qcutoff):
    ...
    line2 = [ (rec, sequence) for sequences in line2_dict.values() for rec, sequence in sequences]
    # print(line2)
